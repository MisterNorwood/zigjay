...
pub fn build(b: *std.Build) void {
    ...
    const wp0 = b.addModule("wp0", .{
        .root_source_file = b.path(b.pathJoin(&.{ "src", "wp0", "wp0" ++ ".zig" })),
        .target = target,
        .optimize = optimize,
    });
    libraries.wp0.linkTo(wp0);
    wp0.addImport("compat", compat);

    const wp0_test = b.addTest(.{
        .root_module = wp0,
    });
    test_step.dependOn(&b.addRunArtifact(wp0_test).step);
...
pub const libraries = struct {
    pub const wp0: Library = .{
        .system_libraries = &.{"wireplumber-0.5"},
    };
